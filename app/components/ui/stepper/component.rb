...
module Ui
  module Stepper
    class Component < Ui::Base
      attr_reader :current_step, :extra_classes

      renders_many :steps, Ui::Stepper::StepComponent
      renders_many :panels

      def initialize(current_step: 1, class: nil, **_opts)
        @current_step = current_step
        @extra_classes = binding.local_variable_get(:class)
      end

      def root_classes
        cx("w-full", @extra_classes)
      end

      # Returns :completed, :current, or :upcoming for a 1-based step index
