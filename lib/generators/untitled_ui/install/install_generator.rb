  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      TAILWIND_REQUIRED_LINES = [
        ...
        '@import "./untitled_ui/sources.css";',
      ...
      def create_tailwind_source_file
        gem_components_path = UntitledUi.gem_root.join("app", "components")
        gem_views_path = UntitledUi.gem_root.join("app", "views")
        base_path = app_path("app/assets/tailwind/untitled_ui")
        relative_components = gem_components_path.relative_path_from(base_path)
        relative_views = gem_views_path.relative_path_from(base_path)
        source_content = <<~CSS
          /* Auto-generated by UntitledUi — tells Tailwind to scan gem files for classes */
          /* Regenerate with: rails generate untitled_ui:install */
          @source "#{relative_components}/**/*.erb";
          @source "#{relative_components}/**/*.rb";
          @source "#{relative_views}/**/*.erb";
        CSS
        target = "app/assets/tailwind/untitled_ui/sources.css"
        target_path = File.join(destination_root, target)
        FileUtils.mkdir_p(File.dirname(target_path))
        File.write(target_path, source_content)
        say_status :create, target, :green
      end

      def migrate_existing_components
        local_components = app_path("app/components/ui")
        return unless local_components.directory?

        component_dirs = Dir.glob(local_components.join("*/")).select { |d| File.directory?(d) }
        return if component_dirs.empty?

        say ""
        say "Note: Found existing components in app/components/ui/", :yellow
        say "These local copies will take precedence over the gem's versions."
        say "To use the gem's versions instead, remove the local copies."
        say "To selectively override, keep only the components you've customized."
        say ""
      end
      ...
      def show_instructions
        ...
        say ""
        say "Components are served directly from the gem. Update via `bundle update untitled_ui`."
        say "To customize a specific component, run:"
        say "  rails generate untitled_ui:component button modal  # copies only what you need"
        say ""
